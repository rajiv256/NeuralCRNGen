...
function crn_main(params, train, val; dims=nothing, EPOCHS=10, LR=0.001, tspan=(0.0, 1.0), threshold=0.5, augval=1.0, pos=1.0, neg=0.0)
    ...
    for epoch in 1:EPOCHS
        ...
        for i in eachindex(train)
            ...
            vars["Op"] = max(0.0, yhatval)
            vars["Om"] = max(0.0, -yhatval)
...
function neuralcrn(; DIMS=3)
    ...
    #     redirect_stdout(fileio) do
            ...
            println("===============================")
            vars = crn_main(params_orig, train, train, EPOCHS=200, dims=DIMS, LR=0.01, tspan=tspan, augval=AUGVAL, pos=POS, neg=NEG, threshold=THRESHOLD)
